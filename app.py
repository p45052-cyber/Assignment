...
# Ensure this loads the updated scaler that was just saved
...
# Define the custom ranges requested by the user (these ranges are for display purposes)
duration_min = 1
duration_max = 24
base_fare_min = 1500
base_fare_max = 40000
...
if st.button('Predict'):
    ...
    if not duration or not base_fare or not tax_surcharge or not days_before_departure:
        ...
        try:
            ...
            # Create a DataFrame with only the input features
            # This DataFrame structure should match the features used to fit the scaler
            ...
            # Define the columns that were used to fit the scaler (input features only)
            scaled_features_list = ['Duration (hrs)', 'Base Fare (BDT)', 'Tax & Surcharge (BDT)', 'Days Before Departure']

            # Select only the features from the input data for scaling
            input_features = input_data[scaled_features_list]

            # Scale the input features using the corrected scaler
            scaled_input_data = scaler.transform(input_features)

            # Make a prediction using the model which expects 4 features
